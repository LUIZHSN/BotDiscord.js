...
exports.run = async (bot, message, args) => {
    ...
    if (message.content.split(' ').slice(1).join(' ').length < 1) {
        message.channel.send(`**<:error:776841201298309160> | ${message.author}**, você não escreveu nada.`)
    } else {
        if (message.content.split(' ').slice(1).join(' ').length > 50) {
            message.channel.send(`**<:error:776841201298309160> | ${message.author}**, você ultrapassou o limite de 50 caracteres. Você não quer uma edição feia ne?`)
        } else {
            if (message.member.hasPermission('ATTACH_FILES')) {
                ...
                message.channel.send('🔍 | Processando...').then  (message => {
                    Jimp.read(`https://cdn.discordapp.com/attachments/538711394137407488/567123894956457984/tirinha_baby.png`, function (err, image) {
                        if (err) message.channel.send(`**<:error:776841201298309160> | ${message.author}**, ocorreu um erro ao criar a imagem.`)
                ...
                message.channel.send(`**<:error:776841201298309160> | ${message.author}**, eu não tenho a permissão necessária para fazer isso. \`ATTACH_FILES\` `)
            }
        }
    }
}
